...
func (this *UserController) Regislation() {
	...
	if err := this.sendEmail(name, email, pwd, md5); err != nil { //发送激活邮件
		this.Abort500(my_errors.New("", err))
	}
...
func (this *UserController) sendEmail(name, email, password, md5 string) error {
	...
	smtp.HTML = fmt.Sprintf(`<!DOCTYPE html>
								...
								</html>`, name, date, password, md5, md5)
